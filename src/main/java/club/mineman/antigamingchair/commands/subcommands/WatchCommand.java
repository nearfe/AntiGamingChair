...
public class WatchCommand implements SubCommand {
   ...
   public void execute(Player player, Player target, String[] args) {
      if(target == null) {
         ...
         player.sendMessage(targetData.isPlayerWatching(player)?CC.L_PURPLE + "You are now watching " + CC.YELLOW + target.getName():CC.L_PURPLE + "You are no longer watching " + CC.YELLOW + target.getName());
      }
