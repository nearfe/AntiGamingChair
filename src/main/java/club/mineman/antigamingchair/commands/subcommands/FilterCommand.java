...
public class FilterCommand implements SubCommand {
   ...
   public void execute(Player player, Player target, String[] args) {
      ...
      if(args.length < 2) {
         ...
         phrase1.append(playerData.getFilteredPhrases().toString().replace("[", CC.YELLOW).replace("]", "").replaceAll(", ", "\n"));
         phrase1.append("\n\u00a78 \u00a78 \u00a71 \u00a73 \u00a73 \u00a77 \u00a78 \u00a7r\n");
         ...
         player.sendMessage(CC.L_PURPLE + "The phrase " + CC.YELLOW + phrase + CC.L_PURPLE + " is " + (playerData.isKeywordFiltered(phrase.toLowerCase())?"now filtered":"no longer filtered") + " in your alerts!");
      }
   }
