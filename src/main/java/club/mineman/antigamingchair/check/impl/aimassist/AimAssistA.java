...
public class AimAssistA extends RotationCheck {
    ...
    public void handleCheck(Player player, PlayerUpdateRotationEvent event) {
        ...
        final float diff = Math.abs(event.getTo().getYaw() - event.getFrom().getYaw());
        if (diff > 1.0f && Math.round(diff) == diff) {
            if (diff == this.suspiciousYaw) {
                this.alert(PlayerAlertEvent.AlertType.RELEASE, player, "failed Aim Assist Check A (Experimental). Y " + diff + ".");
                this.ban(player, "Aim Assist Check A");
            }
